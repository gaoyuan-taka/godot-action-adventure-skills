...
func _ready() -> void:
	...
	if Engine.is_editor_hint():
		child_entered_tree.connect(gather_patrol_locations) # 系统规定child_entered_tree信号需要传入一个node类型参数，因此这里gather_patrol_locations自定义函数需要有一个传入变量，但gather_patrol_locations本身又不需要传入什么参数，所以这里传入null
